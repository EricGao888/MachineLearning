...
#     print trainFeature.shape
    ...
    for j in range(epoch):
        ...
        for i in range(trainSize):
            signMatrix = -1*np.ones((10,28*28+1))
...
#                 print(np.argmax(y_true[i]),np.argmax(y_predict[i]))
                signMatrix[np.argmax(y_true[i])] = 1
                W = W + learningRate*(signMatrix*trainFeature[i:i+1,:])
                A = A + W
                cnt += 1
#         print cnt
    return A/float(cnt)
...
#     W = train_model(10000,epoch,0.001)
#     X.append(epoch)
#     Y.append(predict(W,testSet))
#     Z.append(predict(W,trainSetTMP))
# # Data for plotting
...
# for learningRate in [0.0001,0.001,0.01,0.1]:
#     trainSetTMP = trainSet
