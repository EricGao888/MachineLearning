...
while True:
    ...
    for i in range(10):
        W[i] = (1-1.0*learningRate*lam/sampleNum)*W[i] - \
        learningRate*(1.0/sampleNum)*np.sum((predictMatrix[i]-trueMatrix[i]).reshape(1,sampleNum).T*trainFeature,axis=0)
...
#        print("Improvement: ")+str(round(improvement,2))    
    ...
    # if epoch % 200 == 0:
    #     print epoch
    if len(stopList) == 20 and np.average(stopList[:10])-np.average(stopList[10:20]) <= criteria:
        break
