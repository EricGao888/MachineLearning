# python cv_depth.py /Users/ericgao/Documents/2019Spring/CS573/Output/proj4/trainingSet.csv
import random
import pandas as pd
import numpy as np
import math
...
seB = []
seR = []
for depth_entry in depthList:
	print("Depth: %d" % depth_entry)
	f.write(str(depth_entry) + '\n')
	depthThres = depth_entry
	accuracyD = []
	accuracyB = []
	accuracyR = []
	for idx in range(10):
...
ax.errorbar(depthList, resB, yerr=np.array(seB)*0.2, label = "BT Accuracy")
ax.errorbar(depthList, resR, yerr=np.array(seR)*0.2, label = "RF Accuracy")
ax.set(xlabel="Max Depth", ylabel="Accuracy",
       title="Accuracy against Max Depth")
